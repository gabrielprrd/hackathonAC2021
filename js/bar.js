...
import { getLocalStorageTasks } from './storage.js';
...
var taskscountintial = getLocalStorageTasks().length;
var taskscount = taskscountintial;
...
function move() {
  ...
  if (taskscount > 0) {
    ...
    var width = 10;
    var timeThatHavePassedInSeconds =
      timeStarting.getHours() * 60 * 60 +
      timeStarting.getSeconds() +
      timeStarting.getMinutes() * 60;
    var secondInOneDay = timeThatHavePassedInSeconds + 35;
    //var secondInOneDay = 24*60*60;

    var id = setInterval(
      frame,
      ((secondInOneDay - timeThatHavePassedInSeconds) / taskscountintial) * 1000
    );
    function frame() {
